...
def main():
    ...
    RAW_DATASET_DIR = os.path.join(ROOT_DIR, "football-merged-dataset")
    ...
    if not os.path.exists(RAW_DATASET_DIR):
        print(f"❌ Thư mục dữ liệu nguồn không tồn tại: {RAW_DATASET_DIR}")
        return
    ...
    for split in splits:
        img_dir_in = os.path.join(RAW_DATASET_DIR, "images", split)
        if not os.path.exists(img_dir_in):
            continue
            
        print(f"\n--- Đang xử lý tập: {split.upper()} ---")
        img_dir_out = os.path.join(OUTPUT_DATASET_DIR, "images", split)
        lbl_dir_out = os.path.join(OUTPUT_DATASET_DIR, "labels", split)
        os.makedirs(img_dir_out, exist_ok=True)
        os.makedirs(lbl_dir_out, exist_ok=True)
        
        valid_extensions = ('.jpg', '.jpeg', '.png', '.JPG', '.JPEG', '.PNG')
        all_images = [f for f in os.listdir(img_dir_in) if f.endswith(valid_extensions)]
        print(f"Tìm thấy {len(all_images)} ảnh trong thư mục {split}.")
        ...
        for idx, img_name in enumerate(all_images):
            img_path = os.path.join(img_dir_in, img_name)
            img_predictions = []
            ...
            # Áp dụng NMS chéo để loại bỏ các box trùng nhau
            final_predictions = apply_cross_model_nms(img_predictions, iou_threshold=0.6)

            # Nếu không nhận diện được đối tượng nào -> Bỏ qua
            if not final_predictions:
                skipped_count += 1
                continue

            # Sao chép ảnh vào thư mục đầu ra
            shutil.copy(img_path, os.path.join(img_dir_out, img_name))

            # Lưu file nhãn YOLO tương ứng
            label_name = os.path.splitext(img_name)[0] + ".txt"
            label_path = os.path.join(lbl_dir_out, label_name)
        ...
        print(f"✅ Hoàn thành tập {split}: {labeled_count} ảnh được gán nhãn, {skipped_count} ảnh trống bị bỏ qua.")
